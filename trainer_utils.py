...
from models import TabIDSFeatureExtractor
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        # print(self.x.dtype)
        x = self.x.iloc[index - 1:index]   #x = self.x.iloc[index - 1:index]
        # print('x.shape:',x.shape)
        if self.y is not None:
            y = self.y.iloc[index - 1:index]     # y = self.y.iloc[index - 1:index]
        else:
            y = None
        return x, y
...
class TrainCollator:
    ...
    def __init__(self,
        ...
        self.feature_extractor = TabIDSFeatureExtractor(
...
class SupervisedTrainCollator(TrainCollator):
    ...
    def __call__(self, data):
        x = pd.concat([row[0] for row in data])  # index with dataframe
        y = pd.concat([row[1] for row in data])  # index with label
